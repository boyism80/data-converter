...
namespace ExcelTableConverter.Worker.Generator
{
    public class HasAJsonFileGenerator : ParallelWorker<(string FileName, object DataSet), bool>
    {
        ...
        protected override IEnumerable<(string FileName, object DataSet)> OnReady()
        {
            ...
            {
                var dir = Path.Combine(Context.Output, Context.Configuration.JsonFilePath, "go", scopeName);
                ...
                foreach (var (tableName, dataSet) in Context.Completed.GetFlattenedDataSet(scope))
                {
